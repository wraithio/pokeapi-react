...
const generatePokemonData = async (pokemon: string) => {
  ...
  for (let i = 0; i < data.abilities.length; i++) {
    ...
    const abilityObj: AbilityTree = {
      ...
      description: abilityData.effect_entries.length != 1 ? abilityData.effect_entries[1].effect : abilityData.effect_entries[0].effect
    };
...
const generateFavs = async() => {
    ...
    {
        favList.push({name:formatString(favNames[i]), image: await findPicturebyName(favNames[i])})
    }
    return favList
}

export { generatePokemonData,generateFavs};
